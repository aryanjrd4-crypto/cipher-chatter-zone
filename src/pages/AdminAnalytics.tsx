    ...
    <Layout>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Global Echo statistics</p>
        </div>
        ...
        {data?.chartData && (
          <div className="glass rounded-xl p-4 space-y-3">
            <h2 className="text-sm font-medium text-foreground">Views Trend (7 Days)</h2>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={data.chartData}>
                <XAxis dataKey="date" tick={{ fill: 'hsl(240, 5%, 55%)', fontSize: 11 }} axisLine={false} tickLine={false} tickFormatter={(v) => v.slice(5)} />
                <YAxis tick={{ fill: 'hsl(240, 5%, 55%)', fontSize: 11 }} axisLine={false} tickLine={false} />
                <Tooltip contentStyle={{ background: 'hsl(240, 8%, 9%)', border: '1px solid hsl(240, 6%, 20%)', borderRadius: '8px', fontSize: 12 }} />
                <Bar dataKey="views" fill="hsl(0, 84%, 60%)" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        <div className="glass rounded-xl p-4 space-y-3">
          ...
          <div className="space-y-2">
            {data?.topPosts.map((p, i) => (
              <Link key={p.id} to={`/post/${p.id}`} className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-secondary/50 transition-colors">

...
export default function AnalyticsPage() {
  ...
  // Get view chart data for all user posts
  ...
  const engagementRate = data && data.totalViews > 0
    ? (((data.totalUpvotes + data.totalComments) / data.totalViews) * 100).toFixed(1)
    : '0';
    ...
    <Layout>
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Stats for your anonymous identity</p>
        </div>
        ...
        {chartData && chartData.length > 0 && (
          <div className="glass rounded-xl p-4 space-y-3">
            <h2 className="text-sm font-medium text-foreground">Views (Last 7 Days)</h2>
            <ResponsiveContainer width="100%" height={180}>
              <AreaChart data={chartData}>
                ...
                <Tooltip
                  contentStyle={{ background: 'hsl(240, 8%, 9%)', border: '1px solid hsl(240, 6%, 20%)', borderRadius: '8px', fontSize: 12 }}
                  labelStyle={{ color: 'hsl(0, 0%, 95%)' }}
                />
                <Area type="monotone" dataKey="views" stroke="hsl(0, 84%, 60%)" fill="url(#viewGrad)" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        {data?.mostViewed && (
          <div className="glass rounded-xl p-4 space-y-2">
            <h2 className="text-sm font-medium text-foreground">Most Viewed Post</h2>
            <Link to={`/post/${data.mostViewed.id}`} className="text-primary hover:underline text-sm">
